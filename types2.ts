...
const userInput = prompt("enter a title: ");
...
function compare_hands(player: player, dealer: dealer){
    if (player.total_cards_value > 21) {
        ...
        console.log("Dealer busts! ", player.name, " winner");
...
function black_jack(card_deck: deck, player : player, dealer: dealer){
    ...
    if (player.total_cards_value <= 21) {
        ...
        rl.question("1. Hit, 2. Stand, 3. Double Down: ", (choice) => {
            rl.close();        
       if (choice === ("1")){
 ...
 function black_jack_setup(): void {
    ...
    rl.question("Vad heter du? ", (namn) => {
        if (typeof namn === 'string') {
            ...
            rl.question("Bet?: ", (money) => {
                pot += Number(money)
                player.balance -= Number(money)
